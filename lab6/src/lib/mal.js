...
export async function fetchTopNovels(page = 1) {
  return normalizeListResponse(await requestApi(`${API_BASE}/top/manga?type=novel&limit=8&page=${page}`), 'novel', 'novel')
}
...
function writeCachedResponse(url, payload) {
  ...
  try {
    ...
    // Ignore storage failures and fall back to network requests.
  }
}
